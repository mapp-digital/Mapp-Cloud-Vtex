...
export function handleEvents(e: PixelMessage) {
  switch (e.data.eventName) {
    case 'vtex:pageView': {
      ...
      if (
        e.data.routeId !== 'store.product' &&
        e.data.routeId !== 'store.orderplaced' &&
    ...
    case 'vtex:internalSiteSearchView': {
      ...
      window._ti.internalSearch = window._ti.vtex.pageTitle.replace(
        ' - undefined',
        ''
      )
      pageUpdate()
    ...
    case 'vtex:addToCart': {
      ...
      e.data.items.forEach(product => {
        ...
        window._ti.productId = `${window._ti.productId}${product.productId};`
        window._ti.productName = `${window._ti.productName}${product.name};`
        window._ti.productCategory = `${window._ti.productCategory}${product.category};`
        window._ti.productCost = `${window._ti.productCost}${(
      ...
      if (
        window._ti.productQuantity.length > 0 &&
        hasItem(window._ti.productQuantity)
      ) {
        window.wts = window.wts || []
    ...
    case 'vtex:orderPlaced': {
      ...
      e.data.transactionProducts.forEach(product => {
        window._ti.productId = `${window._ti.productId}${product.id};`
        window._ti.productName = `${window._ti.productName}${product.name};`
        window._ti.productCategory = `${window._ti.productCategory}${product.category};`
        window._ti.productCost = `${window._ti.productCost}${(
          product.price * product.quantity
